...
class TimeSeriesDataset(Dataset):
    ...
    def __init__(
        ...
        def _stable_seed(path: str, salt: int = 0) -> int:
            key = f"{sampling_seed}:{salt}:{os.path.abspath(path)}".encode("utf-8", errors="ignore")
            return int(hashlib.sha256(key).hexdigest()[:8], 16)

        self.sample_seeds = None
        if sampling_seed is not None:
            self.sample_seeds = {path: _stable_seed(path, 1) for path in self.file_paths}

        if self.fixed_random_offset or sampling_seed is not None:
            self.random_offsets = {
                path: np.random.default_rng(_stable_seed(path, 2)).uniform(-0.2 * base_lambda, 0.2 * base_lambda)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, str]:
        try:
            ...
            if self.apply_poisson_sampling:
                if self.sample_seeds is not None:
                    np_state = np.random.get_state()
                    np.random.seed(self.sample_seeds[file_path])
                    try:
