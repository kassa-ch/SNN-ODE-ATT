...
def _build_dataloader(file_paths, apply_poisson_sampling, base_lambda, batch_size, shuffle, sampling_seed=None):
    ds = TimeSeriesDataset(
        file_paths=file_paths,
        apply_poisson_sampling=apply_poisson_sampling,
        base_lambda=base_lambda,
        fixed_random_offset=(not shuffle) or bool(apply_poisson_sampling),
        sampling_seed=sampling_seed if apply_poisson_sampling else None,
    )
    return DataLoader(
        ds,
...
def train_single_kfold_run(
    ...
    seed_base = 100000 + int(fold_id or 0) * 100 + int(run_id or 0) * 1000
    train_dl = _build_dataloader(train_paths, apply_poisson_sampling, base_lambda, batch_size, shuffle=True, sampling_seed=seed_base + 1)
    val_dl = _build_dataloader(val_paths, apply_poisson_sampling, base_lambda, batch_size, shuffle=False, sampling_seed=seed_base + 2)
    test_dl = _build_dataloader(test_paths, apply_poisson_sampling, base_lambda, batch_size, shuffle=False, sampling_seed=seed_base + 3)
