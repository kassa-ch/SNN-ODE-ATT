...
# ====== Distance 统一调用入口 ======
# 说明：
# 1) 所有 Distance/*/score.py 都应实现 demo1 兼容接口:
#       calc.fit(train_features, train_sequences=None, mask=None, time=None)
#       calc.calculate_distance(features, sequences=None, mask=None, time=None)
# 2) demo1 只负责训练/测试特征提取、故障线和指标计算；
#    距离公式全部放在 Distance/ 下，和 Mahalanobis 等价替换。
_THIS_FILE_DIR = os.path.dirname(os.path.abspath(__file__)) if "__file__" in globals() else os.getcwd()
_CANDIDATE_ROOTS = [
    os.getcwd(),
    _THIS_FILE_DIR,
    os.path.dirname(_THIS_FILE_DIR),
    os.path.dirname(os.path.dirname(_THIS_FILE_DIR)),
]
for _root in _CANDIDATE_ROOTS:
    if _root and os.path.isdir(os.path.join(_root, "Distance")) and _root not in sys.path:
        sys.path.insert(0, _root)

try:
    from Distance.distance_factory import create_distance_calculator, available_methods
except Exception as _distance_import_error:
    create_distance_calculator = None
    available_methods = lambda: tuple()
    _DISTANCE_IMPORT_ERROR = _distance_import_error
else:
    _DISTANCE_IMPORT_ERROR = None
...
DETECTION_CONFIG = {
    'n_components': 1,
    'batch_size': 8,
    # 可选：stable_mahalanobis / mahalanobis / euclidean / rich_global_local /
    # sobolev_h1 / sobolev_hminus1 / gaussian_w2 / sliced_wasserstein /
    # sinkhorn_divergence / mmd_cov_kernel / hamiltonian_energy /
    # ode_residual_energy / trace_distance / qre / bures /
    # von_neumann_entropy / purity
    'distance_method': 'stable_mahalanobis',
    'robust_covariance': False,
    # Distance 通用超参数；未被某个方法使用时会被安全忽略
    'distance_reg': 1e-6,
    'distance_standardize': True,
    'rich_alpha': 1.0,
    'rich_beta': 0.5,
    'rich_topk': 5,
...
def extract_distance_feature_cache(model, dataloader, device, feature_type='global', win_size=WIN_SIZE):
    """
    为 Distance/* 统一提取检测特征。
    ...
    返回:
        {
          'z_mean': [N,H],             # 点/全局距离使用
          'z_seq':  [N,T,H],           # 轨迹距离使用，优先 mem_reset_seq
          'mask':   [N,T],
          'time':   [N,T],             # 由 delta_t 累积得到
          'delta_t':[N,T],
        }

    注意：
    - 该函数不使用标签，不参与阈值调参，避免泄漏。
    - demo1 的旧 Mahalanobis 只用 z_mean；新 Distance 方法可直接用 z_seq。
    """
    model.eval()
    z_mean_list, z_seq_list, mask_list, time_list, delta_t_list = [], [], [], [], []
    model_name = getattr(model, "__class__", type(model)).__name__
    ...
    with torch.no_grad():
        for x, m, delta_t, _ in dataloader:
            ...
            if feature_type == 'startup':
                z_seq_use = z_seq[:, :win_size, :] if z_seq.shape[1] >= win_size else z_seq
                z_mean_use = z_seq_use.mean(dim=1)
            else:
                z_mean_use = z_mean
                z_seq_use = z_seq

            # m 在旧 data_loader 中就是有效观测 mask；若维度异常，退化为全 1
            if m.dim() == 3:
                # 兼容 [B,T,D] mask，压缩到 [B,T]
                mask_bt = (m.abs().sum(dim=-1) > 0).float()
            elif m.dim() == 2:
                mask_bt = m.float()
            else:
                mask_bt = torch.ones(z_seq_use.shape[:2], device=device)

            # delta_t 可能是 [B,T] 或 [B,T,1]
            if delta_t.dim() == 3:
                delta_bt = delta_t.squeeze(-1)
            else:
                delta_bt = delta_t
            if delta_bt.dim() != 2:
                delta_bt = torch.ones(z_seq_use.shape[:2], device=device)

            # 用累计 delta_t 构造时间坐标，供 Sobolev / ODE residual 等轨迹方法使用
            time_bt = torch.cumsum(torch.clamp(delta_bt, min=0.0), dim=1)

            z_mean_list.append(z_mean_use.detach().cpu().numpy())
            z_seq_list.append(z_seq_use.detach().cpu().numpy())
            mask_list.append(mask_bt.detach().cpu().numpy())
            delta_t_list.append(delta_bt.detach().cpu().numpy())
            time_list.append(time_bt.detach().cpu().numpy())

    if not z_mean_list:
        raise ValueError("No features extracted for distance calculation.")

    cache = {
        'z_mean': np.vstack(z_mean_list).astype(np.float64),
        'z_seq': np.concatenate(z_seq_list, axis=0).astype(np.float64),
        'mask': np.concatenate(mask_list, axis=0).astype(np.float64),
        'delta_t': np.concatenate(delta_t_list, axis=0).astype(np.float64),
        'time': np.concatenate(time_list, axis=0).astype(np.float64),
    }

    detection_logger.log(
        'data_loading',
        f"Distance特征提取完成: z_mean={cache['z_mean'].shape}, "
        f"z_seq={cache['z_seq'].shape}, mask={cache['mask'].shape}"
    )
    return cache
